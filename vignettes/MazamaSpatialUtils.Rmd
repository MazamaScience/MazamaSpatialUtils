...
* `getCountry~(longitude, latitude, ...)` -- returns names, ISO codes and other country-level data specified lo cations
...
A generic `getSpatialData(longitude, latitude, ...)` returns a dataframe whose rows are associated with specified locations.
 ...
 * `  2 Mb EEZCountries.rda` -- country boundaries including Exclusive Economic Zones
 * ` 15 Mb NaturalEarthAdm1.rda` -- state/province/oblast level boundaries	 
 * ` 61 Mb OSMTimezones.rda` -- Open Street Map high resolution time zones
 * `  4 Mb TMWorldBorders.rda` -- high resolution country level boundaries
 * ` 48 Mb TerrestrialEcoregions.rda` -- terrestrial eco-regions
 * `  7 Mb USCensus115thCongress.rda` -- US congressional districts
 * `  2 Mb USCensusCounties.rda` -- US county level boundaries
 * `  3 Mb USCensusStates.rda` -- US state level boundaries
 * `  3 Mb USIndianLands.rda` -- US tribal boundaries
 * ` 16 Mb WorldTimezones.rda` -- high resolution timezones
...
large and can be downloaded separately from http://data.mazamascience.com/MazamaSpatialUtils/Spatial_0.7/.

 * ` 38 Mb WBDHU2.rda` -- USGS level 2 watersheds
 * `107 Mb WBDHU4.rda` -- USGS level 4 watersheds
 * `136 Mb WBDHU6.rda` -- USGS level 6 watersheds
 * `294 Mb WBDHU8.rda` -- USGS level 8 watersheds
 * `768 Mb WBDHU10.rda` -- USGS level 10 watersheds	 
 * `1.4 Gb WBDHU12.rda` -- USGS level 12 watersheds
...
latitudeitude <- c(47.5, 40.75, 52.1, 48.5)

# Get countries/codes associated with locations
getCountry(longitude, latitudeitude)
getCountryCode(longitude, latitudeitude)

# Review all available data
getCountry(longitude, latitudeitude, allData=TRUE)
```
...
same `latitude` and `longitude` variables as above and assign states to those locations.
...
countryCodes <- getCountryCode(longitude, latitudeitude)

# Pass the countryCodes as an argument to speed everything up
getState(longitude, latitudeitude, countryCodes = countryCodes)
getStateCode(longitude, latitudeitude, countryCodes = countryCodes)

# This is a very detailed dataset so we'll grab a few important columns
states <- getState(longitude, latitudeitude, allData=TRUE, countryCodes = countryCodes)
...
getTimezone(longitude, latitudeitude)

# Get country codes associated with locations
countryCodes <- getCountryCode(longitude, latitudeitude)

# Pass the countryCodes as an argument to potentially speed things up
getTimezone(longitude, latitudeitude, countryCodes = countryCodes)

# Review all available data
getTimezone(longitude, latitudeitude, allData=TRUE, countryCodes = countryCodes)
```
...
stateCodes <- getStateCode(longitude,latitudeitude)

# Optionally pass the stateCodes as an argument to speed everything up
getUSCounty(longitude, latitudeitude, stateCodes=stateCodes)
getUSCounty(longitude, latitudeitude, allData=TRUE, stateCodes=stateCodes)
```
