...
testthat::test_that("allData returns are correct dimension and type", {
  ...
  testthat::expect_equal(dim(getStateCode(-100, 48, allData = TRUE)), c(1,ncol(NaturalEarthAdm1) - 1)) # SFDF has an extra 'geometry' column
  testthat::expect_s3_class(getStateCode(c(-80, -90), c(40, 41.38), allData = TRUE), "data.frame")
  testthat::expect_equal(dim(getStateCode(c(-80, -90), c(40, 41.38), allData = TRUE)), c(2,ncol(NaturalEarthAdm1) - 1)) # SFDF has an extra 'geometry' column
