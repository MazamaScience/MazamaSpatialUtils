...
buffer_zone <- buffer(point_4, width=500000)
...
Clipping works a bit like a cookie-cutter.  It is done using the _intersect()_ function which takes an input geometry and calculates the spatial intersection with another geometry.  Here we will clip the pink "spdf_abcd" feature and the green "spdf_e" feature with the "buffer_zone" we created above. 
...
It is very important to note that the new "_clip" features contain all of the attributes which were contained in the "spdf_abcd" and "spdf_e" SpatialPolygonsDataFrames.  In otherwords, the feature being clipped retained all of its original attribute information.
...
This is ONLY because the "buffer_zone" feature lacked a data frame of its own and was simply a SpatialPolygon object, and not a SpatialPolygonsDataFrame.  
...
```
The behavior of the _intersect()_ function is very different when both features are SpatialPolygonsDataFrames.  When 2 SpatialPolygonsDataFrames are intersected, the default behavior of the _intersect()_ function is to combine the attributes from both data sets.  Which brings us to the next type of operation.

## Intersection

The intersection operation computes the spatial intersection between 2 features and returns a new geometry that is this intersection area.  It also combines the feature attributes of both features.  In the example below, we are intersecting the 2 clipped features with each other.  Note the attributes and the geometry that are returned.

```
combined_clips <- intersect(abcd_clip, e_clip)
...
If we look at just the attributes from "abcd_clip" (a few lines above) we see that it only contains attributes from the original "spdf_abcd" feature set.
...
```
zoned_tracts <- aggregate(spdf_abcd, by='zone')
```
...
zoned_tracts <- aggregate(spdf_abcd, 
                                  by='zone')
...
## 2    2 tract      1 3.5
```
This can be an exceptionally useful feature. 
...
```{r spatial_union, fig.width=7, fig.height=4, message=FALSE, echo=FALSE}
...
Note that _over()_ returns a data frame with the same number of rows as the input feature that contains the matching attribute data from the feature being unioned.  Since here we requested that the "temp" data be aggregated by mean, we get back a table of mean temperatures for each polygon.
...
```
Having to join the data back in this fashion means that if you are doing the same types of analysis often, it may be advantageous to create a helper function that takes in 2  SpatialDataFrames and returns a new one with the attribute data already attached.