...
buffer objects.  Often a buffer is used to define a zone around a specific point.  Here, we will create a 250,000 meter buffer around the _temp_df_ point feature with "id = 2".

```
# Subset a single point out of set
point_2 <- subset(temp_df, id == 2)
...
One problem, or perhaps "side-effect" of creating buffers is that the resulting feature will lack any sort of attribute information.  This can be rectified by using the _intersect()_ and/or _over()_ functions.  

The _over()_ function returns the attributes 

_intersect()_ function returns the geographic intersection of 2 features AND the attributes that belong to each of them.  In the case of our buffer zone, we would like to assign it the attributes of the point from which it was built.  For So for example, if we intersect the new "buffer_zone" polygon (which lacks attributes) with the "point_2" feature, we get back a polygon with all of the point's attributes.



```{r}
# SpatialPointsDataFrame with attributes
dplyr::glimpse(point_2@data)

# SpatialPolygon with no attributes
summary(buffer_zone)

# Create a dataframe with a "distance" attribute/column/field
bz_df <- data.frame(distance = "250000")

# Convert the SpatialPolygon into a SpatialPolygonsDataFrame and attach attributes
buffer_zone <- SpatialPolygonsDataFrame(buffer_zone, bz_df)

point_2_zone <- sp::over(buffer_zone, point_2)
dplyr::glimpse(point_2_zone)
class(point_2_zone)
typeof(point_2_zone)
```
